...
class MPU:
    def __init__(self):
        ...
        # Finds calibration values set by data_setup script (none found, then 0)
        try:
            with open('/home/pi/shared/scripts/python/zero_val.csv') as infile:
                reader = csv.reader(infile, delimiter=',')
                readings = next(reader)
                self.roll_offset = int(readings[1])
                self.pitch_offset = int(readings[2])
        except:
            self.roll_offset = 0
            self.pitch_offset = 0
...
def main():
    ...
    # Infinit Loop
    while True:
        # Attempt to write, offset by zero
        try:
            # Sets up MPU each time, originally the MPU would disconnect after some time and needed to reconnect
            mpu.set_up()
            mpu.complementary_filter()
            roll = round((mpu.roll - mpu.roll_offset),1)
            pitch = round((mpu.pitch*-1) - mpu.pitch_offset,1)              
            with open('/var/www/html/data/tilt.csv', "w") as outfile:
                writer = csv.writer(outfile)
                writer.writerow([(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")),roll, pitch, "Y"])
        except:
            # Attempt fails, attempt to write previous value
            try:
                with open('/var/www/html/data/tilt.csv', "w") as outfile:
                    writer = csv.writer(outfile)
                    writer.writerow([(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")),roll, pitch, "N"])
            except:
                # Attempt fails write 0
                with open('/var/www/html/data/tilt.csv', "w") as outfile:
                    writer = csv.writer(outfile)
                    writer.writerow([(datetime.datetime.now().strftime("%d/%m/%y %H:%M:%S")),0, 0, "N"])
